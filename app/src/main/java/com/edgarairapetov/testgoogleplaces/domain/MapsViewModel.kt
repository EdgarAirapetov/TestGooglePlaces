package com.edgarairapetov.testgoogleplaces.domain
...
import com.edgarairapetov.testgoogleplaces.common.GoogleConstants.Companion.WALKING_MODE
import com.edgarairapetov.testgoogleplaces.data.api.wrapper.place.PlaceResultTypesEnum
...
class MapsViewModel @Inject constructor(var context: Context) : ViewModel() {
    ...
    private val maxPlacesOnMap = 12

    var placesLiveData: MutableLiveData<ArrayList<PlaceResult>> = MutableLiveData()
            ...
            try {
                ...
                for (type in typeSet) {
                    ...
                    body?.let { placesResponse ->
                        placesResponse.type = PlaceResultTypesEnum.valueOf(type)
                        placesResponse.results?.let { results ->
                            results.forEach { result ->
                                result.type = PlaceResultTypesEnum.valueOf(type)
                            }
                        }
                ...
                val arrayPlaces = arrayListOf<PlaceResult>()

                while (arrayList.isNotEmpty()) {
                    val neededElementsCount = (maxPlacesOnMap - arrayPlaces.size) / arrayList.size
